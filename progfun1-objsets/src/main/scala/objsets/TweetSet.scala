...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      ...
      if (p(elem)) { 
       ...
       new NonEmpty(elem,left.filterAcc(p,acc),right.filterAcc(p,acc)).remove(elem)
    ...
    def mostRetweeted: Tweet = {
    	   try {
    		   if ((elem.retweets > right.mostRetweeted.retweets) 
    				   && (elem.retweets > left.mostRetweeted.retweets)){
    			   elem
    		   }
    		   else if ((right.mostRetweeted.retweets > elem.retweets) 
    				   && (right.mostRetweeted.retweets > left.mostRetweeted.retweets)) {
    			   right.mostRetweeted
    		   }
    		   else {
    			   left.mostRetweeted
     ...
     def descendingByRetweet: TweetList  = {
       val t = mostRetweeted
       new Cons(t,left.union(right).remove(t).descendingByRetweet)
