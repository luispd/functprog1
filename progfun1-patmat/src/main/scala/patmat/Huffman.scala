...
object Huffman {
   ...
   */
    // @author Luis
    def timesSingleChar(c: Char, chars:List[Char]): (Char,Int,List[Char]) = chars match {
      case Nil => (c,0,chars)
      case p::resto => {
                         val a = timesSingleChar(c,resto)
                         if (c.equals(p)) {
                           (c,1+a._2,a._3)
                         }
                         else {
   ...
   */
    ...
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
      case Nil => Nil
      case p::resto => insert(p,makeOrderedLeafList(resto))
    }
   ...
   */
    // @author Luis
    def insert2(x: Fork, xs: List[CodeTree]): List[CodeTree] = xs match {
      case List() => List(x)
      case y :: ys => if (x.weight<weight(y)) x::xs else y::insert2(x,ys)
    }
    // @author Luis
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case Nil => trees
      case p::Nil => trees
      case p::s::resto => insert2(Fork(p,s,chars(p):::chars(s),weight(p)+weight(s)),resto)
    }
   ...
   */
   // @author Luis
    def decodeChar(tree: CodeTree, bits: List[Bit]): (Char,List[Bit]) = tree match {
      ...
      case t: Fork => if (bits.head == 0) decodeChar(t.left,bits.tail)
                      else decodeChar(t.right,bits.tail)
    }
    // @author Luis
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = bits match {
      case Nil => Nil
      case bs => {val a= decodeChar(tree,bs)
                   a._1::decode(tree,a._2)
      }
    }
   ...
   */
   def contiene(tree: CodeTree, c: Char) : Boolean = tree match {
     case f: Fork => f.chars.contains(c)
     case l: Leaf => l.char == c
   }

   def encodeAux(tree: CodeTree, c: Char): List[Bit] = tree match {
     case f: Fork => if (contiene(f.left,c)) {
                       0::encodeAux(f.left,c)
                     }
                     else {
                       1::encodeAux(f.right,c)
                     }
     case l: Leaf => Nil
   ...
   def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
     if (text.isEmpty) {
       ...
       encodeAux(tree,text.head):::encode(tree)(text.tail)
     }
   }
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = text match {
      case p::resto => codeBits(convert(tree))(p):::quickEncode(tree)(resto)
      case Nil => Nil
    }
  }
